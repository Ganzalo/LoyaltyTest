 ...
 * Такая архитектуры была выбрана потому что все взаимодействие с пользователем
 * идет через консоль и в связи с малой функциональностью программы.
 *
 * Также есть пакет util c классом Console которые помогает в работе с консолью.
 *
 * Взаимодействия UI с Model идет через интерфейс AverageVoteCalculator.
 ...
 * Решение: создать новый класс CalculatorThread, так как плохая пракика смешивать бизнес логику и потоки
 * (легче проверять работу по отдельности).
 * LPCalculatorImpl через который происходит взаимодействия с классами Model.
 *
 * Calculator необходимо получать данные, поэтому Calculator имеет поля класс DataHolder.
 *
 * Поскольку мы имеем 2 источника данных по разным endpoint, в пакете dataholder было
 * созданно: Constants, FilmsInfoHolder, GenresHolder классы.
 ...
 * FilmsInfoHolder вытаскивает данные за фильмы по страницам (есть желание сделать статиком
 * и перенсти логику DataHolder).
 * Поскольку желательно иметь одну точку взаимодействия с классами FilmsInfoHolder и GenresHolder был
 * созданн DataHolder. Через DataHolder получаем данный для обработки.
 *
 ...
 * по фильмам.
 *
 * С классоми из пакет dataholder взаимодействия происходит через класс DataHolder остальный классы default visible.
 * Классы из пакет vo публичные.
 * С классоми из пакет model взаимодействия происходит через класс LPCalculatorImpl остальный классы default visible.
 *
 *
 */
public class Main {
